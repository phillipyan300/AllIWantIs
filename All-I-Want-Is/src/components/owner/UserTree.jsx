...
function UserTree() {
  ...
  const calculateSpots = () => {
      if (treeRef.current) {
        ...
        for (let row = 0; row < rows; row++) {
          ...
          const rowY = rect.bottom - maxHeight * (row / rows);
  ...
  const handlePlaceOrnament = (ornament) => {
    ...
    // Update the state with the new ornament
    setPlacedOrnaments([
      ...placedOrnaments,
      { ...ornament, position: nextSpot },
    ]);

    // Close the ornament panel
